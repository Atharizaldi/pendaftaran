...
@section('contentMain')
    ...
    <a href="{{ route('ubahpendaftaran', $account->user_id) }}">
        <button
            class="py-2 px-5 w-full text-base font-medium text-center text-white bg-gradient-to-br from-blue-500 to-blue-400 hover:scale-[1.02] shadow-md shadow-gray-300 transition-transform rounded-lg ">Ubah
            Pendaftaran
        </button>

        <form action="{{ route('hapusPendaftaran', $account->user_id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <button type="submit"
                class="py-2 px-5 mt-3 w-full text-base font-medium text-center text-white bg-gradient-to-br from-red-500 to-red-400 hover:scale-[1.02] shadow-md shadow-gray-300 transition-transform rounded-lg ">Hapus
                Pendaftaran
            </button>

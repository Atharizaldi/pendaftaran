...
class AdminController
{
    ...
    public function ubahPendaftaran($user_id)
    {
    ...
    public function detailPendaftaran($user_id)
    {
        ...
        return view('admin.detail', compact('account', 'provinces', 'cities', 'agamas'));
    }

    public function hapusPendaftaran($user_id)
    {
        try {
            ...
            // Redirect ke halaman list akun pengguna dengan pesan sukses
            return redirect()->route('listakunpengguna')->with('success', 'Pendaftaran berhasil dihapus');
        } catch (\Exception $e) {
            // Tangkap error jika terjadi
    ...
    public function updatependaftar(Request $request, $user_id)
    {
        ...
        return redirect()->to("/detailpendaftaran/{$user_id}")->with('success', 'Data Pendaftaran Berhasil Dirubah');
    }
    ...
    public function updatepengguna(Request $request, $userId)
    {
        try {
            ...
            $validatedData = $request->validate([
                ...
                'password_initial' => 'nullable|string|min:5',
            ...
            if ($request->filled('password') || $request->filled('password_initial')) {
    ...
    public function regispeserta(Request $request)
    {
        try {
            ...
            if ($validatedData['password'] === $validatedData['password_initial']) {
                ...
                return Redirect::to('/listakunpengguna')->with('success', 'Berhasil menambahkan pengguna');
            }else{
                return back()->with('error', 'Password tidak sama');
            }
